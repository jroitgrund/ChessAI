...
public abstract class Piece {
  ...
  List<Coord> getMoves_Direction(ArrayList<Coord> l, Board b, Coord from,
      int first_iteration, int second_iteration) {
    for (int i = first_iteration; i + from.getCol() < 8
        && i + from.getCol() >= 0 && i + from.getRow() < 8
        && i + from.getRow() >= 0; i += Integer.signum(first_iteration)) {
      for (int j = second_iteration; j + from.getCol() < 8
          && j + from.getCol() >= 0 && j + from.getRow() < 8
          && j + from.getRow() >= 0; j += Integer.signum(second_iteration)) {
